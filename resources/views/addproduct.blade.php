    ...
    <div class="flex flex-col space-y-4 my-4">
      ...
      <div class="grid xs:grid-cols-1 xs:gap-4 place-items-center sm:grid-cols-3 ">
        ...
        <div class="relative inline-block hover:ring-2 p-4">
          <img id="imagePreview" src="{{ asset('img/sait/add.svg') }}" class="w-52 h-52 object-cover opacity-55"
            alt="Background Image" >
          <input type="file" name="image_path" id="imageInput" accept="image/*"
            class="absolute inset-0 w-full h-full opacity-0 bg-cursor-pointer cursor-pointer " required>
        </div>
        <script>
        ...
        <input type="hidden" id="selected-category-id" name="category_id">
        <input type="hidden" id="selected-manufacturer-id" name="brand_id">
        <!-- Блок с текстом -->
        ...
        <div class="w-hull flex flex-col justify-center">
          <div class="w-56 border-2 rounded-3xl">
            <div class="px-4 w-full h-11 flex justify-between items-center space-x-1 text-xl font-semibold">
              <input placeholder="Цена" name="price" type="text"  pattern="^[0-9]*$" title="Введите неотрицательное число"
                class="ring-0 border-0 bg-opacity-0 focus:outline-none focus:ring-0 focus:border-0 w-24" required>
              <p>₽</p>
            </div>
          ...
          <div class="w-56 mt-2 flex h-11">
            ...
            <input type="text"  pattern="^[0-9]*$" title="Введите неотрицательное число" class="  h-full w-full border border-gray-300 rounded-lg text-center" placeholder="Количество" name="count" required>
      ...
      <div>
        <x-secondary-button class="fixed bottom-10 left-32 p-3" type="submit">Создать товар</x-secondary-button>
      ...
      @component('components.adddescription', [
  'sections' => $sections
])
      @endcomponent
    </div>
